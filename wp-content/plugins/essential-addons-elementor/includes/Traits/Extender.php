...
trait Extender
{
    ...
    // Advanced Data Table
    public function advanced_data_table_source($source)
    {
    ...
    public function advanced_data_table_database_html($settings, $html)
    {
        ...
        } else if ($settings['ea_adv_data_table_source'] == 'remote') {
        ...
        } else if ($settings['ea_adv_data_table_source'] == 'tablepress' && is_plugin_active('tablepress/tablepress.php')) {
            $tables_opt = get_option('tablepress_tables', []);
            $tables_opt = json_decode($tables_opt, true);
            $tables = $tables_opt['table_post'];

            if($settings['ea_adv_data_table_source_tablepress_table_id']) {
        ...
        if (!empty($results)) {
            if ($settings['ea_adv_data_table_source'] == 'google') {
                $html .= '<thead><tr>';
